...
export default function CorporatePartyScreen({ navigation }) {
  ...
  const fetchServiceTypes = async () => {
    try {
      const res = await API.get("/services/service-types");

      const grouped = res.data.reduce((acc, curr) => {
        if (!acc[curr.service_name]) acc[curr.service_name] = [];
        acc[curr.service_name].push(curr);
        return acc;
      }, {});

      const packages = [
        { id: 1, name: "Annual Office Treat", services: grouped },
        { id: 2, name: "Tech Innovation Summit", services: grouped },
      ];

      setEventPackages(packages);
      setLoading(false);
    } catch (error) {
  ...
  const toggleSubService = (packageId, subServiceId) => {
    setSelectedSubServices((prev) => {
      const key = `${packageId}-${subServiceId}`;
  ...
  const additionalCost = useMemo(() => {
    ...
    eventPackages.forEach((pkg) => {
      Object.values(pkg.services).forEach((subServices) => {
        subServices.forEach((sub) => {
          if (selectedSubServices[`${pkg.id}-${sub.id}`]) {
    ...
    return total;
  }, [selectedSubServices, eventPackages]);
  ...
  const handleBookEvent = async () => {
    try {
      const bookingData = {
        user_id: 1, // TODO:  get while logging in and store in AsyncStrorage
        event_id: 2, // TODO: get event id by api
      ...
      if (res.data.success) {
        ...
        navigation.navigate("MyBookings"); // change to your booking list screen
      } else {
        console.log("Not in success 1");
        // Alert.alert("Error", res.data.message || "Booking failed.");
        Alert.alert("Booking", res.data?.message || "Booking successful");

      }
    } catch (error) {
      console.error("Error ala re ala",error);

      // Alert.alert("Error", "Something went wrong while booking.");
      Alert.alert("Error", error.res?.data?.message || error.message || "Something went wrong");
    }
  };
  ...
  return (
    <View style={styles.screen}>
      <FlatList
        ...
        contentContainerStyle={{ paddingBottom: 80 }} // space for sticky bar
        renderItem={({ item }) => (
          <Card style={styles.card}>
            ...
            {expanded[item.id] &&
              Object.entries(item.services).map(([serviceName, subServices]) => (
                <View key={serviceName} style={styles.serviceBlock}>
                  <Text style={styles.serviceName}>{serviceName}</Text>
                  {subServices.map((sub) => {
                    const isChecked =
                      selectedSubServices[`${item.id}-${sub.id}`] || false;
                    return (
                      <View key={sub.id} style={styles.subServiceRow}>
                        <CheckBox
                          value={isChecked}
                          onValueChange={() =>
                            toggleSubService(item.id, sub.id)
                          }
                          color={Colors.buttonBackground}
                        />
                        <Text style={styles.subServiceText}>
                          {sub.service_type_name} - ₹{sub.price}
                        </Text>
                      </View>
                    );
                  })}
      ...
      <View style={styles.bottomBar}>
        <Text style={styles.totalText}>
          Book Event (Corporate Party) - ₹{totalPrice.toLocaleString("en-IN")}
        </Text>
...
export default function CorporatePartyScreen({ route, navigation }) {
  const { categoryId, categoryName } = route.params;
  const [loading, setLoading] = useState(true);
  const [subCategories, setSubCategories] = useState([]);
  const [services, setServices] = useState([]);
  const [expanded, setExpanded] = useState({});
  const [selectedServices, setSelectedServices] = useState({}); // key: `${subCatId}-${serviceId}`
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.bottomBar}>
        <Text style={styles.totalText}>
          Grand Total: ₹
          {(() => {
            const expandedId = Object.keys(expanded).find(id => expanded[id]); // first expanded subcat
            return expandedId
              ? subCategoryTotals[expandedId].toLocaleString("en-IN")
              : 0;
          })()}
        </Text>
        <TouchableOpacity
